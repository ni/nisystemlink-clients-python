...
class QueryJobsRequest(JsonModel):
    ...
    """
    Gets or sets the filter criteria for jobs or systems. Consists of a string of queries composed using AND/OR operators.
    String values and date strings need to be enclosed in double quotes. Parenthesis can be used around filters to better
    define the order of operations. Filter syntax: '[property name][operator][operand] and [property name][operator][operand]'
    ...
    Valid job properties that can be used in the filter:
        ...
        result.return: An array of objects representing return values for each executed function. Example: result.return[0].Contains("Success")
        result.retcode: An array of integers representing code values for each executed function. Example: result.retcode
        result.success: An array of booleans representing success values for each executed function. Example: result.success.Contains(false)
    """
    ...
    Examples: - 'new(id,jid,state)' - 'new(id,jid,config.user as user)' - 'new(id,jid,state,lastUpdatedTimestamp,metadata.queued as queued)'
    """
