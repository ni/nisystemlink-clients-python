...
class TestTestPlans:
    ...
    def test__get_test_plan__returns_get_test_plan(self, client: TestPlansClient):
        get_test_plan_response: TestPlan = client.get_test_plan("1568982")

        assert get_test_plan_response is not None
        assert isinstance(get_test_plan_response, TestPlan)
        assert get_test_plan_response.id == "1568982"
    ...
    def test__create_test_plan__returns_created_test_plans(
        ...
        client.delete_test_plans(
            test_plan_ids=[test_plan_create_response.createdTestPlans[0].id]
        )
